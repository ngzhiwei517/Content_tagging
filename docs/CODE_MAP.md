...
app.py
  ...
  -> ugc_tagger/apify_usage_guard.py (paid-fallback admission)
...
| `ugc_tagger/apify_usage_guard.py` | Shared-beta usage checks, paid-call admission, process lock and private owner alerts | Apify thresholds, concurrency admission or alert behavior needs to change |
...
7. Five user-facing workflow pages: Add Posts, Select Posts, Run, Review and
   Summary & Export. Their internal step IDs remain 2-6 because the old
   credential page was removed from the marketing workflow.
...
- Direct retrieval: start in `direct_post_scraper.py`; paid fallback admission
  belongs in `apify_usage_guard.py`.
