...
Checkpoints are local by default. Supabase/Postgres can optionally provide recovery after an app restart or redeployment and coordinate a global single-worker tagging queue. Each completed post is stored separately, and transient Postgres `57014` cancellations are retried with bounded backoff. Checkpoints never contain provider credentials or downloaded media. See [Persistent checkpoints](docs/PERSISTENT_CHECKPOINTS.md).
