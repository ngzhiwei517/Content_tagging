...
class CsvCompatibilityTests(unittest.TestCase):
    ...
    def test_mixed_platform_file_can_be_scoped_to_the_selected_platform(self):
        text = (
            "Link,Market,Track Name\n"
            "https://www.tiktok.com/@alpha/video/7600000000000000001,MY,Track A\n"
            "https://www.instagram.com/reel/DExampleAbC1/,SG,Track B\n"
        )
        frame = self.read_any_table(UploadedFile("mixed_posts.csv", text.encode("utf-8")))
        tiktok_rows, _ = self.standardize_file_rows(
            frame,
            "mixed_posts.csv",
            platform=TIKTOK,
        )
    ...
    def test_pasted_links_can_be_scoped_to_the_selected_platform(self):
        text = (
            "https://www.tiktok.com/@alpha/video/7600000000000000001\n"
            "https://www.instagram.com/reel/DExampleAbC1/\n"
        )
        self.assertEqual(
            [platform_for_url(link) for link in self.parse_links(text, platform=TIKTOK)],
            [TIKTOK],
        )
        self.assertEqual(
            [
                platform_for_url(link)
                for link in self.parse_links(text, platform=INSTAGRAM_REELS)
            ],
            [INSTAGRAM_REELS],
        )

    def test_add_posts_ui_separates_platform_inputs_but_keeps_one_batch(self):
        self.assertIn('"Platform to add"', APP_SOURCE)
        self.assertIn("[TIKTOK, INSTAGRAM_REELS]", APP_SOURCE)
        self.assertIn('st.tabs(["Upload post files", "Paste post links"])', APP_SOURCE)
        self.assertIn(
            "standardize_file_rows(df, f.name, platform=add_platform)",
            APP_SOURCE,
        )
        self.assertIn("links = parse_links(link_text, platform=add_platform)", APP_SOURCE)
        self.assertIn("append_to_batch(combined_upload)", APP_SOURCE)
        self.assertIn("append_to_batch(pd.DataFrame(rows))", APP_SOURCE)
