...
class MetricsOnlyUiContractTests(unittest.TestCase):
    def test_add_posts_page_offers_metrics_only_and_run_page_uses_selection(self):
        ...
        self.assertIn('"Fetch metrics for ranking"', step_four)
        self.assertIn('else "Fetch metrics" if metrics_only_mode_v68_86', step_four)
        ...
        self.assertIn("selected_posts_preview(\n        batch,\n        apply_top_n=False,", step_three)
        self.assertIn("pasted_links_requiring_metrics(", step_three)
        self.assertIn("ranking_refresh_candidates_v68_96", step_three)
        self.assertIn("if ranking_missing_count_v68_95 > 200:", step_three)
        self.assertIn("Large run reminder:", step_three)
        self.assertIn('"Fetch metrics and rank posts"', step_three)
        self.assertIn("purpose=METRICS_ONLY_PURPOSE_RANKING_V68_95", step_three)
        self.assertIn("purpose=METRICS_ONLY_PURPOSE_EXPORT_V68_95", step_three)
        self.assertIn("if metrics_complete_v68_86:", step_four)
        self.assertIn("_finish_ranking_metrics_run_v68_95", step_four)
    ...
    def test_metrics_state_is_restart_safe(self):
        ...
        self.assertIn('"metrics_only_purpose_v68_95"', state_keys)
        self.assertIn('"metrics_only_previous_analysis_mode_v68_95"', state_keys)
