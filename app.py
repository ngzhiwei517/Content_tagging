...
def direct_tiktok_profile_followers_v68_65(
    normalized_creator_key: str,
    representative_post_url: str,
) -> int:
    """Cache one successful free TikTok public-metadata follower lookup."""
...
def missing_tiktok_follower_targets_v68_65(rows: pd.DataFrame) -> pd.DataFrame:
    """Return unique TikTok creators whose batch follower count is missing."""
    columns = ["Platform", "Creator", "Creator Key", "Link"]
    if not isinstance(rows, pd.DataFrame) or rows.empty:
        return pd.DataFrame(columns=columns)
    if "Platform" not in rows.columns or "Creator" not in rows.columns:
        return pd.DataFrame(columns=columns)
    followers = (
        rows["Followers"].map(clean_num)
        if "Followers" in rows.columns
        else pd.Series(0, index=rows.index, dtype="int64")
    )
    source = rows.copy()
    if "Link" not in source.columns:
        source["Link"] = ""
    targets = source.loc[
        source["Platform"].map(safe_str).eq(TIKTOK) & followers.le(0),
        ["Platform", "Creator", "Link"],
    ].copy()
    targets["Creator Key"] = targets["Creator"].map(creator_key)
    targets = targets[targets["Creator Key"].ne("")]
...
def _melodyiq_report_queue_v68_100() -> List[Dict]:
    ...
    legacy_id = (
        safe_str(legacy_report.get("reportId"))
        if isinstance(legacy_report, dict)
        else ""
    )
    if legacy_id and not any(
        safe_str(value.get("report_id")) == legacy_id for value in queue
    ):
        queue.append(
            {
                "report_id": legacy_id,
...
def _melodyiq_save_report_queue_v68_100(queue: List[Dict]) -> None:
    st.session_state.melodyiq_reports_v68_100 = [
        dict(value) for value in queue if isinstance(value, dict)
    ]


def _melodyiq_remove_report_v68_100(report_id: str) -> None:
    _melodyiq_save_report_queue_v68_100(
        [
            value
            for value in _melodyiq_report_queue_v68_100()
            if safe_str(value.get("report_id")) != safe_str(report_id)
        ]
    )


def _melodyiq_report_key_v68_100(report_id: str) -> str:
...
def _melodyiq_import_plan_caption_v68_101(plan: Dict) -> str:
    ...
    if mode == "all":
        return "All available posts (preview or download only)"
    sort_label = next(
        (
            label
...
def _render_melodyiq_import_plan_controls_v68_101(
    ...
    *,
    ...
    if selected_mode == "top":
        ...
        st.caption(
            "Preview or download the complete report. To add rows to Current "
            "batch, choose Top posts or Latest posts."
        )

    if show_report_note:
        st.caption("Choose which posts are added to Current batch.")
    return {
        "mode": selected_mode,
        "limit": int(limit),
        "sort_field": sort_field,
...
def _render_melodyiq_report_progress_v68_99(api_key: str) -> None:
    ...
    pending = [
        ...
        if not _melodyiq_report_ready_v68_97(value.get("report", {}))
    ...
    queue_index, entry = pending[cursor % len(pending)]
    report_id = safe_str(entry.get("report_id"))
    try:
        report = MelodyIQClient(api_key).get_report(report_id)
        queue[queue_index] = {**entry, "report": report}
        _melodyiq_save_report_queue_v68_100(queue)
        st.session_state.melodyiq_poll_cursor_v68_100 = (cursor + 1) % max(
            len(pending), 1
        )
...
def _render_melodyiq_report_card_v68_100(
    ...
    """Render actions for one independently managed standard report."""
    report_id = safe_str(entry.get("report_id"))
    report = entry.get("report") if isinstance(entry.get("report"), dict) else {}
    report_key = _melodyiq_report_key_v68_100(report_id)
    track_value = safe_str(entry.get("track")) or "Untitled track"
    artist_value = safe_str(entry.get("artist"))
    if not artist_value:
        report_artists = report.get("artists") or []
        artist_value = safe_str(report_artists[0]) if report_artists else ""
    ready = _melodyiq_report_ready_v68_97(report)
    tiktok_report = report.get("tktk") if isinstance(report.get("tktk"), dict) else {}

    label = f"{track_value} · {'Ready' if ready else 'Preparing'}"
    with st.expander(label, expanded=ready):
        if artist_value:
            st.caption(f"Artist: {artist_value}")
        status_col, count_col = st.columns(2)
        with status_col:
            st.metric("Report status", "Ready" if ready else "Preparing")
        with count_col:
        ...
        if not ready:
                ...
                if st.button(
                    ...
                    try:
                        if import_plan["mode"] == "all":
                            ...
                            preview = _melodyiq_import_rows_v68_97(
                            ...
                            posts = client.get_all_impactful_posts(
                                report_id,
                                limit=min(int(import_plan["limit"]), 20),
                                sort_field=sort_field,
                                sort_direction=(
                                    "asc" if sort_field == "rank" else "desc"
                                ),
                            )
                            preview = _melodyiq_import_rows_v68_97(
                                impactful_posts_frame(posts),
                        ...
                        st.session_state[preview_key] = preview.head(20)
                    except MelodyIQError as exc:
                        st.error(str(exc))
            with action_columns[1]:
                if export_url:
                    ...
                    st.button(
                        ...
                        disabled=True,
                        width="stretch",
                        key=f"melodyiq_download_unavailable_{report_key}",
                    )
            ...
            if import_plan["mode"] == "all":
                st.info(
                    "All posts can be previewed or downloaded. Choose Top posts "
                    "or Latest posts to add a bounded set to Current batch."
                )
            else:
                delete_after = st.checkbox(
                    ...
                    help=(
                        ...
                        "the shared standard-report limit."
                    ),
                ...
                if st.button(
                    ...
                    try:
                        ...
                        st.session_state.last_message = (
                            f"Added {added:,} MelodyIQ rows. "
                            f"Skipped {skipped:,} duplicate rows.{cleanup_warning}"
                        )
                        st.rerun()
                    except MelodyIQError as exc:
                        st.error(str(exc))
        ...
        if st.button(
            "Delete report",
            width="stretch",
            key=f"melodyiq_delete_report_{report_key}",
            help="Delete this report from MelodyIQ without changing Current batch.",
        ):
            try:
                client.delete_report(report_id)
                _melodyiq_remove_report_v68_100(report_id)
        ...
        if not sounds:
            ...
            if st.button(
                ...
                try:
                    ...
                    queue.append(
                        {
                            ...
                            "report": report,
                            "track": safe_str(
                                st.session_state.get("melodyiq_track_v68_97")
                            ),
                            "artist": safe_str(
                    ...
                    st.error(str(exc))
            st.caption(
                "You can create multiple reports at the same time. Choose Top, "
                "Latest, or All posts after each report is ready, then delete the "
                "report after import to free a report slot."
            )
...
def prepare_creative_type_engagement_chart_data_v68_70(
    df: pd.DataFrame,
    *,
    max_categories: int = 12,
) -> pd.DataFrame:
    """Aggregate post count and mean engagement rate for each creative type."""
    ...
    summary = (
        ...
        .agg(
            ...
            **{"Average Engagement Rate": ("Engagement Rate", "mean")},
        )
        .reset_index()
        .sort_values(
...
def render_creative_type_bar_chart_v68_49(mix: pd.DataFrame) -> None:
    ...
    if mix is None or mix.empty or "Average Engagement Rate" not in mix.columns:
        ...
        return
    chart_data = mix.copy()
    chart_data["Posts"] = pd.to_numeric(chart_data["Posts"], errors="coerce").fillna(0).clip(lower=0)
    ...
    if chart_data.empty:
        st.markdown("<div class='empty-panel'>Engagement-rate chart will appear after metrics are available.</div>", unsafe_allow_html=True)
        return
    if px is None:
        chart_bar(
            chart_data,
    ...
    fig.update_layout(
        ...
        showlegend=False,
        height=500,
        margin=dict(l=52, r=22, t=56, b=128),
        ...
        xaxis=dict(
            title="Creative type",
            categoryorder="array",
            categoryarray=chart_data["Creative Type"].tolist(),
            tickangle=-45,
            automargin=True,
            showgrid=False,
            zeroline=False,
...
if st.session_state.step == 2:
    st.markdown("<div class='card page-heading'><h2>Add posts</h2><p class='sub'>Find posts by track, upload files, or paste post links into one batch.</p></div>", unsafe_allow_html=True)
...
# STEP 6: Summary and export
    ...
    missing_follower_targets = missing_tiktok_follower_targets_v68_65(work)
    if not missing_follower_targets.empty:
        attempted_follower_keys = {
            safe_str(key)
            for key in st.session_state.get(
                "tiktok_follower_attempted_keys_v68_65", []
            )
            if safe_str(key)
        }
        pending_follower_targets = missing_follower_targets[
            ~missing_follower_targets["Creator Key"].isin(attempted_follower_keys)
        ]
        if pending_follower_targets.empty:
            attempted_follower_keys = set()
            pending_follower_targets = missing_follower_targets.copy()
            st.session_state.tiktok_follower_attempted_keys_v68_65 = []
        with st.container(border=True):
            follower_info_col, follower_action_col = st.columns([2, 1])
            with follower_info_col:
                st.markdown("**Missing TikTok followers**")
                st.caption(
            ...
            if fill_followers_clicked:
                ...
                if successful_followers:
                    _persist_runtime_checkpoint_v68_15()
                    st.success(
                        f"Filled follower counts for {successful_followers:,} creator"
                        f"{'s' if successful_followers != 1 else ''} across "
                        f"{filled_rows:,} post{'s' if filled_rows != 1 else ''}."
                    )
                if failed_followers:
                    st.warning(
                        f"{failed_followers:,} public profile"
                        f"{'s' if failed_followers != 1 else ''} did not return a follower count. "
    ...
    with c1:
        with st.container(border=True):
            ...
            mix = prepare_creative_type_engagement_chart_data_v68_70(
                filtered,
                max_categories=12,
            )
